...
@Component({
  selector: 'app-pos-testing-execution',
  templateUrl: './pos-testing-execution.component.html',
  styleUrls: ['./pos-testing-execution.component.scss']
})
...
export class PosTestingExecutionComponent implements OnInit,OnDestroy {
  downloadFile() {
...
//// 
    ...
    this.router.events.subscribe(event => {
      ...
      if (event instanceof NavigationStart) {
         ...
         if (event.url=="/user/iso/testing-execution") {
          // console.log("url OK");
         }else if (event.url !="/user/pos/testing-execution" && this.ButtonConf==true) {
          ...
          window.location.reload();
             ...
             if (this.isConnected) {
              // console.log('url KO');
              this.router.navigateByUrl('/user/pos/testing-execution');
              this.openModalConf();
             }
         }
  ...
  handleCheckboxChange(t: string) {
    ...
    if(t=="echoTestAutoReply"){
      ...
      if(this.echoTestAutoReply){
        ...
        const filteredData = data.map((item: { id: any; caseSetDesc: any; posCasesDefinitions: any[]; }) => ({
                id: item.id,
                caseSetDesc: item.caseSetDesc,
                casesDefinitions: item.posCasesDefinitions.filter(def => def.caseDirection === "req" && def.caseAction === "ET")
            }))
            .filter((item: { posCasesDefinitions: string | any[]; }) => item.posCasesDefinitions.length > 0);
  ...
  getSenarios() {
    ...
    this.globalService.getScenariosPOS(id).subscribe(
      data => {
          // console.log('data: ', data);
  ...
  selectCase(id:any,scenarioId:any){
    ...
    cases.posCasesDefinitions.map((item:any)=>{
...
// }
    ...
    if (this.isConnected) {
      ...
      // // console.log("amount===>",this.amount);
        ...
        if (this.isSelectCase) {
          ...
          this.selectedScenario.posCasesDefinitions.forEach((caseItem: any) => {
  ...
  createFieldsAction(fields: any[]) {
    ...
    ChangeActionSend() {
      ...
      const filteredData = data
          .map((item: { id: any; caseSetDesc: any; posCasesDefinitions: any[]; }) => ({
              id: item.id,
              caseSetDesc: item.caseSetDesc,
              posCasesDefinitions: item.posCasesDefinitions.filter(def =>  def?.caseAction === this.selectedOption)
          }))
          .filter((item: { posCasesDefinitions: string | any[]; }) => item.posCasesDefinitions.length > 0);
 ...
 if (this.isConnected) {
    ...
    prepareMessageForCaseAction(filteredData:any, scenarioToSend:any, index:any) :{     header: string;     messageOut: string;     messageIn: string;     index: number;     id: string;     scenario: string; } {
      ...
      // console.log("excution api  getBuffer -> ");
        this.globalService.getBufferPOS(data,this.protocol).subscribe({
          next: (data1: any) => {
            ...
            if (data1.respCode === "000") {
              ...
              this.messages.id = data.posCasesDefinitions.id
 ...
 ////  **************** **********************///////////
  ...
  if (this.isSelectScenario) {
    ...
    const filteredCases = body.posCasesDefinitions.filter((caseDef: { id: { caseNo: string; }; }) => caseDef.id.caseNo === caseNum);
...
// console.log("caseNum v3",caseNum);
    ...
    if (this.isSelectCase) {
      const filteredBody = this.scenarios.find((scenario: { posCasesDefinitions: any[]; }) => {
        return scenario.posCasesDefinitions.some((definition: { id: { caseNo: any; }; }) => definition.id.caseNo === data.id.caseNo);
      });
        body = {
        id: filteredBody.id,
        caseSetDesc: filteredBody.caseSetDesc,
        posCasesDefinitions: filteredBody.posCasesDefinitions.filter((definition: { caseDesc: any; }) => {
    ...
    // console.log("this cardNo==>",this.caseForm.get("caseCardRef")?.value);
       ...
       body.posCasesDefinitions.forEach((definition: { caseCardRef: string;  caseMerchantPrf: string; caseTerminalPrf: string;  fields: any[]; }) => {
    ...
    if(data.id.caseType==="I"  )
     ...
     this.globalService.getBufferPOS(body,this.protocol).subscribe({
        next: (data1: any) => {
          // console.log("data getBuffer -> ",data1);
          if (data1.respCode === "000") {
            const resultData = JSON.parse(data1.result);
  ...
  getDataToShow(dataToShow:any){
    ...
    let table=[]
    this.globalService.getDataToShowPOS(dataToShow,this.protocol,this.bankCode).subscribe({
      next: (data: any) => {
        // // console.log("show data  ",data);
  ...
  selectTransaction(row: any) {
    ...
    if (req == item.bufferReq) {
      ...
      this.globalService.getSidDumpPos(body).subscribe((data) => {
      ...
      this.globalService.SidMsgHexTracePos(body).subscribe((data) => {
        // console.log('data: ', data);
  ...
  getFields() {
    ...
    this.globalService.getFieldsPOS().subscribe((data) => {
...
////////
  ...
  getOneInstitutionParam(){
      ...
      this.globalService.getOneInstitutionParamPOS(body).subscribe(res=>{
        if(res.respCode==="000"){
  ...
  getAllComms() {
    ...
    this.globalService.getOneCommunicationParamPOS(id).subscribe({
      next: (data: any) => {
        // console.log(data);
...
getNameCase(data: string): string {
  ...
  for (const scenario of this.scenarios) {
    ...
    for (const caseDefinition of scenario.posCasesDefinitions) {
