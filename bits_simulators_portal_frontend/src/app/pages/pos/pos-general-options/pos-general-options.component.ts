...
@Component({
  selector: 'app-pos-general-options',
  templateUrl: './pos-general-options.component.html',
  styleUrls: ['./pos-general-options.component.scss']
})
    export class PosGeneralOptionsComponent {
    ...
    item: any;
      ...
      ngOnInit() {
        ...
        if (this.language === "en") {
          ...
          this.en=true
            ...
            Promise.resolve().then(() => this.globalService.titleSubject.next("Paramètres / General"));
            this.fr=true
      ...
      getCurrencyDescBase(baseCcy: string): string {
        const currency = this.currencyList.find((currency: { id: string; }) => currency.id === baseCcy);
        return currency ? currency.ccyDesc : baseCcy;
      }
    
      getCurrencyDesc(baseCcy: string): string {
        const currency = this.currencyList.find((currency: { id: string; }) => currency.id === baseCcy);
        return currency ? currency.ccyDesc : baseCcy;
      }
    
    
      getFieald(subfldType: string): string {
        const field = this.list_Fields.find((field: { id: { fieldId: { toString: () => string; }; }; }) => field.id.fieldId.toString() === subfldType);
        return field ? subfldType+"-"+field.fieldName  : subfldType;
      } 
      ...
      if (this.isDelete) {
        this.globalService.updateMccPOS(this.mccForm.value).subscribe(response => {
         this.closemodal()
          // console.log('response://// ', response);
          if (response.respCode == '000') {
            this.mccForm.reset();
        ...
        this.closemodal()
        this.globalService.addMccPOS(this.mccForm.value).subscribe(response => {
          if (response.respCode == '000') {
            this.exchangeForm.reset();
            this.mccForm.reset();
            this.getMccList()
      ...
      this.globalService.deleteMccPOS(this.idMcc.value).subscribe(response => {
        this.closemodal();
        if (response.respCode == "000") {
          this.getMccList()
      ...
      if (this.isDelete) {
        this.globalService.updateResponsePOS(this.responseForm.value).subscribe(response => {
         this.closemodal()
          // console.log('response://// ', response);
          if (response.respCode == '000') {
            this.responseForm.reset();
        ...
        this.closemodal()
        this.globalService.addResponsePOS(this.responseForm.value).subscribe(response => {
          if (response.respCode == '000') {
            this.responseForm.reset();
            this.getResponseList()
            this.showBtnDelete=false
      ...
      this.globalService.deleteResponsePOS(this.idResponse.value).subscribe(response => {
        this.closemodal();
        if (response.respCode == "000") {
          this.getResponseList()
    ...
    getFields(){
      ...
      this.globalService.getFieldsPOS().subscribe(response => {
    
        // console.log('response: ', response);
        if (response.respCode ==="000" ) {
          this.list_Fields = response.result
        }
      })
    }
      ...
      if (this.isDelete) {
        this.globalService.updateAccountPOS(this.accountForm.value).subscribe(response => {
         this.closemodal()
          // console.log('response://// ', response);
          if (response.respCode == '000') {
            this.accountForm.reset();
        ...
        this.closemodal()
        this.globalService.addAccountPOS(this.accountForm.value).subscribe(response => {
          if (response.respCode == '000') {
            this.accountForm.reset();
            this.getAccountList()
      ...
      deleteAccountRecord() {
      this.globalService.deleteAccountPOS(this.idAccount.value).subscribe(response => {
        this.closemodal();
        if (response.respCode == "000") {
          this.getAccountList()
