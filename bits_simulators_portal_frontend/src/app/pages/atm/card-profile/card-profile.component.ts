...
import {ERROR_MESSAGES} from "../../../../assets/error/error";
...
export class CardProfileComponent {
  cardPGeneralForm: FormGroup = new FormGroup({
    cardNumber: new FormControl('', [Validators.required, Validators.minLength(13), Validators.maxLength(20)]),
    cardDesc: new FormControl('', [Validators.required]),
    bin: new FormControl('', [Validators.required, Validators.maxLength(3)]),
  ...
  ngOnInit() {
    ...
    if (this.language === "en") {
      Promise.resolve().then(() => this.globalService.titleSubject.next("Acquirer Profile"));
      this.en=true
  ...
  getCards() {
    ...
    this.atmService.getCards(body).subscribe(data => {
...
////  fin encrypt KEY ********************************
 getCurrencyList() {
        ...
        this.cards.map((item: any) => {
          if (item.id == id) {
            ...
            if (this.cardPGeneralForm.get('cardType')?.value != 'MG') {
              ...
              this.atmService.fetchIcc(iccBody).subscribe(response => {
  ...
  deleteCard() {
    ...
    this.atmService.deleteCard(id).subscribe(response => {
      this.closeModalCard();
  ...
  saveCard() {
    ...
    this.globalService.validateCardExp(bodyValidateCardExp).subscribe(response => {
      if (response.respCode=="000") {
        ...
        if (this.isDelete) {
          this.atmService.updateCard(body).subscribe(response => {
            this.closeModalCard()
            // console.log('response://// ', response);
            if (response.respCode == '000') {
              this.getCards()
          ...
          this.atmService.addCard(body).subscribe(response => {
            // // console.log('response: ', response);
            if (response.respCode == '000') {
              this.getCards()
  ...
  addicProfiles() {
    if (this.isUpdate) {
      ...
      this.icProfileObject.id.iccProtocol = ""
  ...
  deleteTagRecord(id: any) {
    ...
    this.atmService.deleteIcc(id).subscribe(response => {
      // console.log('response: ', response);
...
//////
  ...
  openSavePopUpCard(){
    // console.log("this.cardPGeneralForm.valid",this.cardPGeneralForm.valid);
    // console.log("this.magneticForm.valid",this.magneticForm.valid);
    if (
      this.cardPGeneralForm.valid
