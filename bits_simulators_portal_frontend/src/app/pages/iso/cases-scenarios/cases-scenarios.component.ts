...
@Component({
  selector: 'app-cases-scenarios',
  templateUrl: './cases-scenarios.component.html',
  styleUrls: ['./cases-scenarios.component.scss']
})
export class CasesScenariosComponent implements OnInit {
  ...
  // we need a function that takes normal fields and add details for them depending on the field code
  // and also add colapsed : true field if its required
  // we will make it in a seperate service that will be used to handle data changes like this
  ...
  constructor(
    ...
    this.globalService.protocol.subscribe(val => {
      if (val) {
        ...
        this.protocoleId = val
      }
    })
    if (!this.protocoleId) {
      this.protocoleId = localStorage.getItem('protocol') as string

      // console.log( "protocol V2",this.protocoleId)
    }
 ...
 changebuttonRadioOut(){
  ...
  retrieveFields(): void {
    const body={
      "fieldProtocole": this.protocoleId,
      "bankCode": this.bankCode
    }
    this.casesscenariosService.getFields(body).subscribe({
      next: (data: any) => {
        // console.log(data);
  ...
  retrieveCases(): void {
    ...
    this.casesscenariosService.getCases(body).subscribe({
      next: (data: any) => {
  ...
  retriveCase(caseNo: string): void {
    ...
    // console.log(body)
    this.casesscenariosService.getCase(body).subscribe({
      next: (data: any) => {
        // console.log(data);
        if (data.respCode === "000") {
  ...
  retrieveMtis(): void {
    ...
    this.globalService.fetchMTI(body).subscribe({
      next: (data: any) => {
        // console.log(data);
        if (data.respCode === "000") {
  ...
  addCase(body: any): void {
    this.casesscenariosService.addCase(body).subscribe({
      next: (data: any) => {
        // console.log(data);
        if (data.respCode === "000") {
  ...
  updateCase(body: any): void {
    this.casesscenariosService.updateCase(body).subscribe({
      next: (data: any) => {
        // console.log(data);
        if (data.respCode === "000") {
          this.isAlertVisible = true;
  ...
  deleteCaseConfirm(): void {
    if (this.tab_1) {
      ...
      this.casesscenariosService.deleteCase(id).subscribe({
        next: (data: any) => {
          // console.log(data);
          if (data.respCode === "000") {
            this.isAlertVisible = true;
      ...
      // console.log("request : ", id);
      this.casesscenariosService.deleteScenario(id).subscribe({
        next: (data: any) => {
          // console.log(data);
          if (data.respCode === "000") {
            this.isAlertVisible = true;
  ...
  retrieveScenarios(): void {
    ...
    this.casesscenariosService.getScenarios(body).subscribe({
      next: (data: any) => {
        // console.log(data);
  ...
  retrieveScenario(id: any): void {
    ...
    this.casesscenariosService.getScenario(body).subscribe({
      next: (data: any) => {
        // console.log(data);
        if (data.respCode === "000") {
  ...
  saveScenario(): void {
    ...
    // console.log(body);
    this.casesscenariosService.saveScenario(this.scenarioMode, body).subscribe({
      next: (data: any) => {
        // console.log(data);
        if (data.respCode === "000") {
          this.isAlertVisible = true;
