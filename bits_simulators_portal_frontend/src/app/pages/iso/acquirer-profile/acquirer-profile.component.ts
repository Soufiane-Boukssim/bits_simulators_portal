...
@Component({
  selector: 'app-acquirer-Profile',
  templateUrl: './acquirer-Profile.component.html',
  styleUrls: ['./acquirer-profile.component.scss']
})
export class AcquirerProfileComponent {
  ...
  getCards() {
    ...
    this.globalService.getCards(body).subscribe(data => {
...
////  fin encrypt KEY ********************************
        ...
        this.cards.map((item: any) => {
          if (item.id == id) {
            ...
            if (this.cardPGeneralForm.get('cardType')?.value != 'MG') {
              ...
              this.globalService.fetchIcc(iccBody).subscribe(response => {
  ...
  this.globalService.deleteCard(id).subscribe(response => {
    this.closeModalCard();
  ...
  this.globalService.validateCardExp(bodyValidateCardExp).subscribe(response => {
    if (response.respCode=="000") {
      ...
      if (this.isDelete) {
        this.globalService.updateCard(body).subscribe(response => {
          this.closeModalCard()
          // // console.log('response://// ', response);
          if (response.respCode == '000') {
            this.getCards()
        ...
        this.globalService.addCard(body).subscribe(response => {
          // // // console.log('response: ', response);
          if (response.respCode == '000') {
            this.getCards()
...
getCity() {
  ...
  getTags() {
    ...
    this.globalService.fetchTagByProtocol(body).subscribe(data => {
      // // console.log('data: ', data);
  ...
  getMerchantList() {
    ...
    // // console.log('id: ', id);
    this.globalService.fetchMerchant(id).subscribe(response => {
  ...
  submitMerchant() {
    ...
    if (this.showBtnDelete) {
      this.globalService.updateMerchant(body).subscribe(response => {
      ...
      this.globalService.addMerchant(body).subscribe(response => {
        // // // console.log('response: ', response);
        this.closeModalGlobal()
        if (response.respCode == '000') {
          this.initMerchant()
  ...
  deleteMerchantRecord() {
    ...
    // // console.log(body);
    this.globalService.deleteMerchant(body).subscribe(response => {
      this.closeModalGlobal();
      if (response.respCode == "000") {
        this.initMerchant();
        this.getMerchantList();
  ...
  deleteTagRecord(id: any) {
    ...
    this.globalService.deleteIcc(id).subscribe(response => {
      // // console.log('response: ', response);
  ...
  getTerminalList() {
    ...
    // // console.log('id: ', id);
    this.globalService.fetchTerminal(id).subscribe(response => {
...
initTerProfil(){
  ...
  submitTerminal() {
    ...
    if (this.showBtnDeleteProfile) {
      this.globalService.updateTerminal(body).subscribe(response => {
      ...
      this.globalService.addTerminal(body).subscribe(response => {
        this.closeModalProfile()
        if (response.respCode == '000') {
          this.initTerProfil()
          this.getTerminalList()
  ...
  // // console.log(body);
  this.globalService.deleteTerminal(body).subscribe(response => {
    this.closeModalProfile();
    if (response.respCode == "000") {
          this.initTerProfil()
          this.getTerminalList()
...
readSmartCardData(): void {
  this.icProfiles=[]
  this.websocketService.send({ header: 'readSmartCard', messageOut: "" });
  ...
  this.websocketService.subscribeToSmartCardTags((smartCardData) => {
    ...
    this.icProfiles = this.alltags.map((tag: { Name: any; Label: any; Value: any; }) => {
          return {
              "id": {
                  "icPrf": null,
                  "icTag": tag.Name,
              ...
              "icPrfDesc":null,
              "icTagDesc": tag.Label,
              "icTagValue": tag.Value
          };
      });
    
      // // console.log("icProfiles de Card  ====>   ",this.icProfiles);
...
onCurrencyChange() {
 ...
 // // console.log(" this.cardPGeneralForm.get('accountCurr') ==>", this.cardPGeneralForm.get('accountCurr')?.value);
 
 // Récupérer la devise sélectionnée
 const selectedCurrency = this.currencyList.find((currency: any) => currency.id === this.cardPGeneralForm.get('accountCurr')?.value);
 // // console.log("test  selectedCurrency",selectedCurrency);


 if (selectedCurrency) {
  const ccyExponent = selectedCurrency.ccyExponent;
  this.cardPGeneralForm.get('accAmount')!.setValue((0).toFixed(ccyExponent));
  this.cardPGeneralForm.get('available')!.setValue((0).toFixed(ccyExponent));
