...
public class IccController {
   ...
   /* @SecurityRequirement(name = "Bearer Authentication")
    ...
    public ResponseEntity<IccProfile> getIccProfile(@RequestBody IccProfileId id) {
        try {
            logger.info("test 1");
            IccProfile iccProfile = posIccService.getIccProfile(id);
            if (iccProfile != null) {
                logger.info("test 2");
  ...
  /*  @SecurityRequirement(name = "Bearer Authentication")
    ...
    public List<IccProfile> getFilteredIccList(
        ...
        return posIccService.getFilteredIccList(bankCode, iccProtocol);
    }*/
  ...
  /*  @SecurityRequirement(name = "Bearer Authentication")
    ...
    public   List<IccProfile> getFilteredIccList(
        ...
        return posIccService.getFilteredIccList(id.getBankCode(), id.getIccProtocol());
    }*/

    @SecurityRequirement(name = "Bearer Authentication")
    @PostMapping("/getAllIccProfiles")
    public ResponseApiJson<List<IccProfile>> getAllIccProfile(@RequestBody IccProfileId id) {
        logger.info("#################### Star getAllIccProfiles ####################");
        ResponseApiJson<List<IccProfile>> responseApiJson = iccService.getAllIccProfiles(id);
   ...
   /* @SecurityRequirement(name = "Bearer Authentication")
    ...
    public ResponseEntity<String> addIccProfile(@RequestBody IccProfile iccProfile) {
       try {
           ...
           IccProfile i = posIccService.addIccProfile(iccProfile);
   ...
   /* @SecurityRequirement(name = "Bearer Authentication")
    ...
    public ResponseEntity<String> deleteIccProfile(@RequestBody IccProfileId id) {
        try {
        logger.info("#################### Star deleteIccProfile ####################");
        IccProfile i = posIccService.deleteIccProfile(id);
        logger.info("#################### End deleteIccProfile ####################");
            if (i == null) {
                return new ResponseEntity<>(HttpStatus.CONFLICT);
            } else {
