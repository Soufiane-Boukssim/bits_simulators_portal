...
import com.simulator.iso.model.*;
...
@Service
public  class WebSeeviceIsoService {
    private final Logger logger = LogManager.getLogger(WebSeeviceIsoService.class);
    @Autowired
    ...
    private Object toBufferIsoApi(String jsonString ,String protocolCode ,String msg ) {
        ...
        if ("SS".equals(protocolCode) || "CS".equals(protocolCode)  || "GS".equals(protocolCode) || "US".equals(protocolCode) ) {
...
//            response = builder.baseUrl(GlobalVars.baseUrlTest)
            response = builder.baseUrl(GlobalVars.baseUrlTest)
                    ...
                    .block();
//            return response;
        }else if ("VS".equals(protocolCode)||"VB".equals(protocolCode)){
            logger.info("protocol to service  ["+protocolCode+"] ");
            response = builder.baseUrl(GlobalVars.baseUrlTest)
                    .build()
                    .post()
                    .uri(GlobalVars.visaBuildMsg)
                    .headers(httpHeaders -> httpHeaders.addAll(globalVars.getHeaders1())) // Set headers go to global Vars to see which headers here
                    .accept(MediaType.APPLICATION_JSON)
                    .body(BodyInserters.fromValue(jsonString)) // Send JSON in the request body
                    .retrieve()
                    .bodyToMono(String.class)
                    .block();
        }else if("MC".equals(protocolCode)||"MD".equals(protocolCode)){
            logger.info(" protocol to service   ["+protocolCode+"] ");
            response = builder.baseUrl(GlobalVars.baseUrlTest)
                    .build()
                    .post()
                    .uri(GlobalVars.masterBuildMsg)
                    .headers(httpHeaders -> httpHeaders.addAll(globalVars.getHeaders1())) // Set headers go to global Vars to see which headers here
                    .accept(MediaType.APPLICATION_JSON)
                    .body(BodyInserters.fromValue(jsonString)) // Send JSON in the request body
                    .retrieve()
    ...
    public ResponseApiJson<?> getBuffer(CasesDefinition casesDefinition,String protocolCode,String msg) {
    ...
    public void setField(CasesDefinition casesDefinition, FieldsDefinitionRepository fieldDefinitionRepo, GlobalVars globalVars,String protocol,String userCode) {
        String field11="";
    ...
    public ResponseApiJson<?> getDataShow(DataToShow dataToShow ,String protocolCode ,String bankCode) {
        String idRequest = "MS_" + new Date().getTime() + (Math.random() * 9999);
        ...
        try {
            Object objectRes;
            Object objectReq;
            AuthoMsgLog authoMsgLog = new AuthoMsgLog();
...
//            logger.info("dataToShow===>"+dataToShow);
            ...
            CardProfileId cardId = new CardProfileId(dataToShow.getReqNoFormater().getField("2"), protocolCode, bankCode);
            CardProfile cardProfile = cardRepository.findById(cardId).orElse(null);
...
//                    authoMsgLog.setAutCaseCardPrf(dataToShow.getResNoFormater().getField("4"));
            ...
            Date date=new Date();
            AuthoMsgLogId id = new AuthoMsgLogId(date, protocolCode, bankCode); // Modify as needed
    ...
    public ResponseApiJson<?> responseMessage(ResponseModel responseModel,String protocolCode, String msg) {
        String idRequest = "MS_" + new Date().getTime() + (Math.random() * 9999);
        try {
            Optional<CasesDefinition> casesDefinitionIn = casesRepository.findById(responseModel.getId());
            responseModel.getId().setCaseType('O');
            Optional<CasesDefinition> casesDefinitionOut = casesRepository.findById(responseModel.getId());
    ...
    private String handleSpecialValues(String value, String fieldNumber, String field11, CasesDefinition casesDefinition,String protocol) throws Exception {
    ...
    private String handleFromCardProfile(String fieldNumber, CasesDefinition casesDefinition) {
        Date date = new Date();
        switch (fieldNumber) {
            case "2":
    ...
    private String handleGenerateValue(String fieldNumber, CasesDefinition casesDefinition, String field11,String protocol) throws InterruptedException {
        Date date = new Date();
    ...
    private String handleInstitution(String fieldNumber, CasesDefinition casesDefinition) {
        switch (fieldNumber) {
            case "41":
                return getTermProfile(casesDefinition.getCaseTerminalPrf(),casesDefinition.getId().getCaseProtocole(),casesDefinition.getId().getBankCode()).getId().getTermCode();
    ...
    private Object WebServiceHexTrace(String messageIn ,String protocolCode) {
        ...
        if ("SS".equals(protocolCode) || "CS".equals(protocolCode) || "GS".equals(protocolCode) || "US".equals(protocolCode)) {
            ...
            String response = builder.baseUrl(GlobalVars.baseUrlTest)
                    .build()
                    .post()
                    .uri(GlobalVars.VisaMsgHexTrace)
                    .headers(httpHeaders -> httpHeaders.addAll(globalVars.getHeaders1())) // Set headers go to global Vars to see which headers here
                    .accept(MediaType.APPLICATION_JSON)
                    .body(BodyInserters.fromValue(requestBody)) // Send JSON in the request body
                    .retrieve()
            ...
            String response = builder.baseUrl(GlobalVars.baseUrlTest)
                    .build()
                    .post()
                    .uri(GlobalVars.MasterCardMsgHexTrace)
                    .headers(httpHeaders -> httpHeaders.addAll(globalVars.getHeaders1())) // Set headers go to global Vars to see which headers here
                    .accept(MediaType.APPLICATION_JSON)
                    .body(BodyInserters.fromValue(requestBody)) // Send JSON in the request body
                    .retrieve()
